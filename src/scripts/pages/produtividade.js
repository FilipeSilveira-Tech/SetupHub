...
async function renderprodutividadeCategory() {
  const container = document.getElementById("produtividade-list");
  if (!container) return;

  const data = JSON.parse(localStorage.getItem("@SetupHub:Produtividade"));
      ...
      <section class="bg-white border-bottom pb-4 pt-4 mb-4">
          <div class="container">
              <div class="d-flex align-items-center gap-3">
                  ...
                  <div>
                      <h2 class="fw-bolder text-dark mb-0">Produtividade</h2>
                      <p class="text-muted mb-0">Programas para ajudar na produtividade e organização pessoal.</p>
                  </div>
              </div>
          </div>
      </section>
  ...
  if (!data) {
    const softwares = await SoftwareService.getByCategory("Produtividade");
    container.innerHTML = "";
...
export { renderprodutividadeCategory };

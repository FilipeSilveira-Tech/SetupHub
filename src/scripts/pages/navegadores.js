...
async function renderBrowserCategory() {
  const container = document.getElementById("browser-list");
  if (!container) return;

  const data = JSON.parse(localStorage.getItem("@SetupHub:Navegadores"));
      ...
      <section class="bg-white border-bottom pb-4 pt-4 mb-4">
          <div class="container">
              <div class="d-flex align-items-center gap-3">
                  ...
                  <div>
                      <h2 class="fw-bolder text-dark mb-0">Navegadores</h2>
                      <p class="text-muted mb-0">Selecione e baixe os melhores navegadores web disponíveis.</p>
                  </div>
              </div>
          </div>
      </section>
  ...
  if (!data) {
    const softwares = await SoftwareService.getByCategory("Navegadores");
    container.innerHTML = "";

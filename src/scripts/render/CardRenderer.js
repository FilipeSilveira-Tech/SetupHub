class CardRenderer {
  ...
  static createInstallButton(software) {
    ...
    button.onclick = async () => {
      console.log("Iniciando instalação de:", software.nome);
      ...
      try {
        // 3. Chama o processo IPC do Electron que criamos nos passos anteriores
        // Supondo que você crie um método install no seu window.api:
  ...
  static createRemoveButton(software) {
    ...
    button.onclick = () => {
      console.log("Instalar", software.nome);
    };
    return button;
  }
  ...
  static createDownloadButton(software) {
    const button = document.createElement("button");
    button.className =
      "btn btn-primary btn-sm w-100 fw-semibold py-2 shadow-sm d-flex align-items-center justify-content-center gap-2";
    button.innerHTML = `<i class="bi bi-download"></i> Download`;
    // Evento de clique (por enquanto apenas para teste)
